...
public class OAuthMvcTest {
    ...
    private String obtainAccessToken(String username, String password) throws Exception {
        ...
        params.add("client_id", "sampleClient");
        params.add("username", username);
        params.add("password", password);

        // @formatter:off

        //http://localhost:8081/spring-security-oauth-server/oauth/authorize?response_type=token&client_id=sampleClientId&redirect_uri=http%3A%2F%2Flocalhost%3A8083%2F&scope=read%20write%20foo%20bar&state=
    ...
    public void testImplicit() {
    	try {
	        final MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
	        params.add("grant_type", "implicit");
	        ...
	        JacksonJsonParser jsonParser = new JacksonJsonParser();
	        System.err.println(jsonParser.parseMap(resultString).get("access_token").toString());
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    }
